...
const Assignedstudents = () => {
  ...
  const getAllAssignedStudents = async () => {
    ...
    // console.log("data", data.assignstudentsDetails.assignStudentsArray[1].data);
    return data;
  };
  useEffect(() => {
    ...
    <React.Fragment>
      <Layout>
        <TABLE.TableWrapper>
          ...
          {!studentList && <PulseLoader size="10" color="#3aafa9" />}
            ...
            studentList.map((studentArray, index) => (
              <TABLE.TableTbody key={studentArray.data.enrollstudentId}>
                <TABLE.TableTRR>
                  ...
                  <TABLE.TableTdd>{studentArray.data.firstName}</TABLE.TableTdd>
                  <TABLE.TableTdd>

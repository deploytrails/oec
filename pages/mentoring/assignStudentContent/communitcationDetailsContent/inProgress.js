...
const InProgressData = ({ studentModelData }) => {
  const [isComData, setIsComData] = useState([]);
  const ProfileId = Cookies.get("employeeID");
  const comModeList = ["Physical Visit", "Over Phone"];
  const comTypeList = ["Parent", "Student"];
  const comActionList = ["Not Started", "In Progress", "Completed"];

  const employeeName = Cookies.get("userName");

  const loadComData = async () => {
    const comData = await getSelectedComDetail(
      studentModelData.enrollstudentId,
      ProfileId
    );
    console.log(comData);
    setIsComData(comData);
  };

  const comCreateSchema = Yup.object().shape({
    commuNo: Yup.number().required(),
    commuDate: Yup.date().required(),
    commuMode: Yup.string(),
    commuType: Yup.string(),
    commuActionItem: Yup.string().required(),
    commuActionStatus: Yup.string(),
    commuDetails: Yup.string().required()
  });

...
const MarksTable = ({ marksData, examType }) => {
  ...
  const uniqueTypes = [];
  marksData
    ?.filter((marks) => marks.examType === examType)
    .map((filteredMarks) => uniqueTypes.push(filteredMarks));
  ...
  return (
    <React.Fragment>
      {uniqueTypes.length > 0 ? (
        ...
        >
          <tr className="block">
            <th className="w-2/12 px-1 py-2 text-left">Course Code</th>
            <th className="w-2/12 px-1 py-2 text-left">Course Name</th>
            <th className="w-2/12 px-1 py-2 text-left">Exam Type</th>
            <th className="w-1/12 px-1 py-2 text-left">Exam Date</th>
            <th className="w-1/12 px-1 py-2 text-left">Semester</th>
            <th className="w-1/12 px-1 py-2 text-left">Section</th>
          </tr>
        ...
        <p className="w-full block bg-white shadow-lg mb-4 border text-center">
          {" "}
          {examType} Exams are not scheduled yet.
        </p>
      )}
      {isMarksView && (
        <ViewMarksEntry

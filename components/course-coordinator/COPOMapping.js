...
const COPOMapping = ({ courseData }) => {
  ...
  const [posLables, setPosLables] = useState([
    "",
          ...
          {posData &&
            posData.map((item, i) => (
              <TABLE.TableTdd>
                <input
                  type="text"
                  value={renderTextTitle(item, index + 1)}
                  className="box-border mt-4 rounded border border-gray-400"
                ></input>
              </TABLE.TableTdd>
            ))}
  ...
  return (
    <React.Fragment>
      ...
      <button
        type="button"
        className=" allign-right bg-yellow-400 block  mx-auto px-2 py-1 rounded"
      >
        SaveAll
      </button>
    </React.Fragment>

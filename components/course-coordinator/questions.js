...
import {
  getExamTypeData,
  getQuestionsData,
} from "../../services/courceCoordinatorAllocationService";
      ...
      .of(
        Yup.object().shape({
          ...
          age: Yup.mixed().when("includeAge", {
            is: true,
            then: Yup.number().nullable().required("Age is required"),
          }),
        })
      )
      .min(1, "Friends is >= 1"),
                    ...
                    {values &&
                      ...
                      values.map((question, mainIndex) => (
                        <TABLE.TableTbody key={question?.questionNumber}>
                          {question &&
                            question?.noOfQuestions.map(
                              (subquestion, subIndex) => (
                                <TABLE.TableTRR
                                  key={subquestion?.questionPaperId}
                                >
                                  <TABLE.TableTdd>
                                  ...
                                  <TABLE.TableTdd>
                                    {subquestion.courseOutcome}
                                  </TABLE.TableTdd>
                                  <TABLE.TableTdd>
                                    {subquestion.bloomsLevel}
                                  </TABLE.TableTdd>
                                  <TABLE.TableTdd>
                                    {subquestion.benchMark}
                                  </TABLE.TableTdd>
                                </TABLE.TableTRR>
                              )
                            )}
